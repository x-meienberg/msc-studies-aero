  ...
  for (int i=0; i < g->n(0); i++) {
    ...
    for (int j=0; j < g->n(1); j++) {
      ...
      for (int k=0; k < g->n(2); k++) {
        int km  = g->periodic_index(k, -1, 2);
        for (int ind=0; ind < g->nv(); ind++) {
          if (a[0]>0.0) g->f(0,i,j,k)[ind] = a[0]*g->u(im,j ,k )[ind];
          else          g->f(0,i,j,k)[ind] = a[0]*g->u(i ,j ,k )[ind];
          if (a[1]>0.0) g->f(1,i,j,k)[ind] = a[1]*g->u(i ,jm,k )[ind];
          else          g->f(1,i,j,k)[ind] = a[1]*g->u(i ,j ,k )[ind];
          if (a[2]>0.0) g->f(2,i,j,k)[ind] = a[2]*g->u(i ,j ,km)[ind];
          else          g->f(2,i,j,k)[ind] = a[2]*g->u(i ,j ,k )[ind];
        }
      }
    }
  }
