...
\begin{itemize}
    ...
    \item $[M_{ael}] = [M]$, $[C_{ael}] = [C] - q [C_a]$, and $[K_{ael}] = [K]-q [K_a]$
...
\newline \newline We want to quantify \emph{aerodynamic lag and reduction of magnitude} by introducing some functions depending on reduced frequency $k$:
 ...
 \begin{itemize}
     \item In time domain: We check the response to a given excitation, through dynamic stability, ABCD Matrices, Numerical (Direct time integration) with response in unsteady excitation or canocial inputs 
