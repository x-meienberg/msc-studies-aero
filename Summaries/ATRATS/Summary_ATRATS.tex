...
\begin{equation}
	G(t) = \sum_{k \in \Gamma} \int_0^t \psi(\tau, k)R_k(\tau, t) d\tau 
\end{equation}

Is the expected value of the reliability (survival function) of the system with the global transition density in convolution. The reliability function is used similar as the methods before, where 1 is added to the counter. We use $R$ as a counter here as well, however we use it as follows for both unavailability and unreliability

\begin{itemize}
	\item Unreliability: $R_k(\tau, t) = 1$
	\item Unavailability: $R_k(\tau, t) = e^{-\lambda^k(t-\tau))}$	
\end{itemize}
